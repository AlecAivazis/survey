...
func TestSelectRender(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			prompt,
			SelectTemplateData{SelectedIndex: 2, Choices: prompt.Options},
			`? Pick your word:
  foo
  bar
❯ baz
...
`,
		},
		{
			...
			prompt,
			SelectTemplateData{Answer: "buz", ShowAnswer: true, Choices: prompt.Options},
			"? Pick your word: buz\n",
		},
		{
...
`,
		},
		{
			...
			helpfulPrompt,
			SelectTemplateData{SelectedIndex: 2, ShowHelp: true, Choices: prompt.Options},
			`ⓘ This is helpful
? Pick your word:
  foo
  bar
